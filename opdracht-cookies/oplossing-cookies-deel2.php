    ...
    if($_POST["username"] == $textFileArray[0] && $_POST["password"] == $textFileArray[1]) {
        ...
        if(isset($_POST["remember"])) {
            if($_POST["remember"] == "on") {
                setcookie("myCookie", $_POST["username"], time()+60*60*24*30); //cookie aanmaken die na 30 dagen vervalt
...
if(isset($_GET["logoutCookie"])) {
    ...
    if($_GET["logoutCookie"] == "delete") {
        ...
        $loggedIn = false;
        $errorVisible = false;
        header("location: oplossing-cookies-deel2.php"); //zodat je geen error krijgt
    }
}
...
<body>
  ...
  <?php if(!$loggedIn) : ?>
   ...
   <p class="error <?php if($errorVisible) {echo "visible";} ?>">Gebruikersnaam en/of paswoord niet correct. Probeer opnieuw.</p>
    ...
    <a href="oplossing-cookies-deel2.php?logoutCookie=delete">Uitloggen</a>
